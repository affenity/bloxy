...
module.exports = async function(universeId, id) {
    var newPromise = new Promise(async function(resolve, reject) {
        ...
        let url = `https://games.roblox.com/v1/games/${universeId}/votes`
        let options = {
            id: id,
            json: true
        }
        ...
        fetch(url, options).then(({res})=>{
            if (res.statusCode!==200) return reject(`Failed to get game votes, status code: ${res.statusCode}, message: ${res.statusMessage}`);
            resolve(res.body);
        }).catch(reject);
    })
    return newPromise;
